  ...
  const handleUpdateStatus = async () => {
    ...
    try {
      await apiRequest(`/findings/${selectedFinding.id}`, {
        ...
        body: JSON.stringify({
          status: updateStatus,
          resolution_notes: feedbackNotes || null,
        }),
      })
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-700 bg-slate-800/50">
                <th className="text-left py-3 px-4 text-xs font-semibold text-slate-400 uppercase tracking-wider">Asset / Komponen</th>
                <th className="text-left py-3 px-4 text-xs font-semibold text-slate-400 uppercase tracking-wider">Temuan</th>
                <th className="text-left py-3 px-4 text-xs font-semibold text-slate-400 uppercase tracking-wider">Prioritas</th>
                ...
                findings.map((f) => (
                  <tr key={f.id} className="hover:bg-slate-700/20">
                    <td className="py-3 px-4">
                      ...
                      <div className="text-xs text-slate-500 mt-0.5">{f.component_name || '-'}</div>
                    </td>
                    <td className="py-3 px-4 text-slate-300 max-w-xs truncate" title={f.description}>{f.description}</td>
                    <td className="py-3 px-4">
                      <span className={`text-xs px-2 py-0.5 rounded border ${f.priority === 'high' ? 'border-red-500/50 text-red-400 bg-red-500/10' : f.priority === 'medium' ? 'border-yellow-500/50 text-yellow-400 bg-yellow-500/10' : 'border-slate-500/50 text-slate-300 bg-slate-500/10'}`}>
                        {f.priority?.toUpperCase() || 'LOW'}
                      </span>
                    </td>
                    <td className="py-3 px-4"><StatusBadge status={f.status} /></td>
                    <td className="py-3 px-4">
                      <div className="text-sm text-slate-200">{f.reported_by?.name || '-'}</div>
                      <div className="text-xs text-slate-500">{f.created_at ? new Date(f.created_at).toLocaleDateString() : '-'}</div>
                      {f.photo_url ? (
      ...
      {statusModal && selectedFinding && (
        <ModalPortal>
          <div onClick={() => setStatusModal(false)}>
            <div className="bg-slate-800 border border-slate-700 rounded-2xl w-full max-w-2xl p-6" onClick={(e) => e.stopPropagation()}>
