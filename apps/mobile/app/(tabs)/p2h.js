...
export default function P2HScreen() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        setTemplate({ id: '1', name: 'P2H Default', items: [{ id: '1', item_name: 'Level Oli Mesin' }] });
      }
      ...
      try {
        ...
        setHistory(rows);
        setTodayLocked(rows.some((x) => isSameDay(x?.created_at, new Date())));
      } catch (_e) {
        setHistory([]);
    ...
    load();
  }, [activeAsset?.id, loadCurrentAssignment, filters]);

  const items = (template?.items || [{ id: '1', item_name: 'Level Oli Mesin' }]).map((item, idx) => ({
    ...item,
  ...
  const submit = async () => {
    ...
    try {
      ...
      await p2hService.submit({ asset_id: String(activeAsset.id), template_id: String(template?.id || '1'), items: payloadItems });
      setFormState({});
      try {
      ...
      showAlert({ title: 'Gagal', message: 'Data P2H belum berhasil dikirim.', type: 'error' });
    } finally {
      setIsLoading(false);
    }
  };
