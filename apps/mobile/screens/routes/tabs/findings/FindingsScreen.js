...
export default function FindingsScreen() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (loadSeq !== loadSeqRef.current) return;
      setRows([]);
      setHistoryError(error?.message || 'Riwayat temuan gagal dimuat.');
    } finally {
  ...
  const submit = async () => {
    ...
    try {
      ...
      showAlert({ title: 'Gagal', message: error?.message || 'Laporan temuan belum berhasil dikirim.', type: 'error' });
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const onDelete = async (id) => {
    ...
    try {
      ...
      showAlert({ title: 'Gagal', message: error?.message || 'Temuan tidak dapat dihapus.', type: 'error' });
    } finally {
      setIsLoading(false);
    }
  };
      ...
      {historyError ? (
        <Card style={styles.historyCard}>
          <View style={{ alignItems: 'center' }}>
            ...
            <Text style={[styles.historyDate, { marginLeft: 0, marginTop: 8, color: theme.colors.error, textAlign: 'center' }]}>{historyError}</Text>
            <TouchableOpacity style={styles.emptyBtn} onPress={() => load()}>
              <Text style={styles.emptyBtnText}>Muat Ulang</Text>
            </TouchableOpacity>
          </View>
        </Card>
      ) : null}
